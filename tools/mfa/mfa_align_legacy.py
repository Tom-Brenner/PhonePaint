...
def resolve_mfa_bin() -> str:
    """Locate the MFA CLI binary (Kaldi sidecar); never import montreal_forced_aligner.

    Resolution order:
      1. MFA_BIN
      2. ``<conda-envs>/<MFA_CONDA_ENV|mfa_env>/bin/mfa`` (sibling of CONDA_PREFIX
         or under ``~/miniconda3`` / ``~/anaconda3``)
      3. ``mfa`` on PATH
    """
    ...
    raise FileNotFoundError(
        "MFA CLI not found. Install mfa_env (./create_mfa_envs.sh) or set MFA_BIN "
        f"to the mfa executable (looked for env {env_name!r})."
    )


def mfa_cli_env(**extra: str) -> dict:
    """Env for subprocesses that run the MFA CLI sidecar.
    ...
    (and their shared libs), which ``conda run -n mfa_env`` used to provide.
    """
    env = os.environ.copy()
...
def run_mfa(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"MFA not found at {mfa_cmd}. Set MFA_BIN or create mfa_env "
            "(./create_mfa_envs.sh)."
        ) from exc
    return out_dir
